...
const Projects = () => {
    ...
    const [cardsToSaw, setCardsToSaw] = useState(1);

    useEffect(() => {
        const updateCardsToSaw = () => {
            if(window.innerWidth >= 1024) {
                setCardsToSaw(projectsData.length)
            } else {
                setCardsToSaw(1)
            };
        }
        ...
        updateCardsToSaw()
        window.addEventListener("resize", updateCardsToSaw);
        return () => window.removeEventListener("resize", updateCardsToSaw);
    }, [])
  ...
  return (
    <div className='container mx-auto py-4 pt-20 px-6 md:px-20 my-20 w-full overflow-hidden' id="Projects">
        ...
        <div className='overflow-hidden'>
            <div className='flex gap-8 transition-transform duration-500 ease-in-out' style={{transform: `translateX(-${(currentIndex * 100) / cardsToSaw}%)`}}>
